...
class SuujiIntroDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function($intro){
                $__route_name_delete = 'suuji.intro.destroy';
                $delete_url = route($__route_name_delete, $intro->id);

                $__route_name = 'suuji.intro.edit';
     ...
     *
     * @param \App\Suuji $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Suuji $model)
    {
        // DB::statement(DB::raw('set @rownum=0'));
        $query = $model->newQuery()
            ->join("master_rooms as room","room.id","=","suuji.room")
            ->select([
                'suuji.id',
                'suuji.room',
                'suuji.page',
